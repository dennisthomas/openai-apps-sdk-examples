...
function widgetDescriptorMeta(widget: TotalWirelessWidget) {
  return {
...
function widgetInvocationMeta(widget: TotalWirelessWidget) {
  return {
    "openai/toolInvocation/invoking": widget.invoking,
    "openai/toolInvocation/invoked": widget.invoked,
  } as const;
...
const widgets: TotalWirelessWidget[] = [
  {
    id: "totalwireless-plans",
    title: "Search Total Wireless Mobile Plans",
    templateUri: "ui://widget/totalwireless-plans.html",
    invoking: "Searching Total Wireless plans",
    invoked: "Displayed Total Wireless plans",
    html: readWidgetHtml("plans"),
    responseText: "Found matching Total Wireless mobile plans!",
  },
  {
    id: "totalwireless-devices",
    title: "Search Total Wireless Devices",
    templateUri: "ui://widget/totalwireless-devices.html",
    invoking: "Searching devices catalog",
    invoked: "Displayed matching devices",
    html: readWidgetHtml("devices"),
    responseText: "Found matching devices in the Total Wireless catalog!",
  },
  {
    id: "totalwireless-stores",
    title: "Find Total Wireless Stores",
    templateUri: "ui://widget/totalwireless-stores.html",
    invoking: "Finding nearby stores",
    invoked: "Displayed store locations",
    html: readWidgetHtml("stores"),
    responseText: "Found Total Wireless store locations!",
  },
];

const widgetsById = new Map<string, TotalWirelessWidget>();
const widgetsByUri = new Map<string, TotalWirelessWidget>();
...
function createTotalWirelessServer(): Server {
  const server = new Server(
    {
      name: "totalwireless-node",
      version: "0.1.0",
    },
    {
      capabilities: {
...
async function handleSseRequest(res: ServerResponse) {
  ...
  const server = createTotalWirelessServer();
...
const httpServer = createServer(
  async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === "GET" && url.pathname === "/health") {
      ...
      res.end(JSON.stringify({ status: "ok", service: "totalwireless-mcp-server" }));
      return;
    }
...
httpServer.listen(port, () => {
  console.log(`Total Wireless MCP server listening on http://localhost:${port}`);
